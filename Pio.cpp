...
void PIO::setNewDeletedDir() {
    deletedDir =
            QFileDialog::getExistingDirectory(this->prv->getWnd(), "Select dir for deleted files", deletedDir) + '/';
...
QString PIO::getCurrentName() const {
    QRegExp exp("/([^/]+)/");

    return exp.lastIndexIn(currentDir) > 0 ? exp.cap(1) : "";
}
